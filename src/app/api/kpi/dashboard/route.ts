...
export async function GET(request: NextRequest) {
  try {
    ...
    const areaName = searchParams.get('areaName'); // กรองตามอำเภอ (optional)
    ...
    const transformedData = kpis.map(kpi => {
      ...
      return {
        ...
        // Aggregated values for debugging
        _grandTotal: reportData?.grandTotal || 0,
        _totalTarget: reportData?.totalTarget || 0,
      };
    });
